        ...
        cells.push([]);
            for(let col=0;col<MAX_COLS;col++){
                cells[row].push({
                    ...
                    state:CellState.open  //TODO: make this open later
                })
            }

    }
    //randomly put N bombs
    let bombsPlaced = 0;
...
//calculate numbers for each cell
    for(let rowIndex=0;rowIndex<MAX_ROWS;rowIndex++){
        for(let colIndex=0;colIndex<MAX_COLS;colIndex++){
            ...
            let numberOfBombs = 0;
            const {topLeftCell,topCell,topRightCell,leftCell,rightCell,bottomCell,bottomLeftCell,bottomRightCell} = grabAllAdjacentCells(cells,rowIndex,colIndex);
...
export const openMultipleCells = (cells:CellType[][],rowParam: number,colParam: number): CellType[][] =>{
    ...
    const {topLeftCell,topCell,topRightCell,leftCell,rightCell,bottomCell,bottomLeftCell,bottomRightCell} = grabAllAdjacentCells(cells,rowParam,colParam);
